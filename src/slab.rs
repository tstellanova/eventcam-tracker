...
//                        else
                        if spatial_dist < best_parent_distance {
                            //println!("{} -> {} dist spatial: {} < {}", evt.timestamp, prior_evt.timestamp, spatial_dist, best_parent_distance);
                            best_parent_distance = spatial_dist;
                            best_parent_time = prior_evt.timestamp;
                            best_parent_opt = Some(nb.clone());
                            best_likeness = likeness;
                            ...
                            if prior_evt.timestamp > best_parent_time {
                                //println!("time better {}  >  {}", prior_evt.timestamp, best_parent_time);
                                best_parent_distance = spatial_dist;
                                best_parent_time = prior_evt.timestamp;
                                best_parent_opt = Some(nb.clone());
                                best_likeness = likeness;
    ...
    fn find_best_parent_cell(&self, evt: &SaeEvent) -> Option<FeatureLocator> {
        let (leaf_neighbors, nonleaf_neighbors) = self.collect_neighbors(evt);
...
//        if best_parent_opt.is_some() {
//            println!("best parent: {:?}", best_parent_opt.unwrap().event);
//        }
    ...
    fn collect_neighbors(&self, evt: &SaeEvent) -> (Vec<FeatureLocator>, Vec<FeatureLocator>) {
    ...
    pub fn add_and_match_feature(&mut self, evt: &SaeEvent) -> Option<FeatureLocator> {
        let time_horizon: SaeTime = std::cmp::max(evt.timestamp, FORGETTING_TIME)  - FORGETTING_TIME;
        ...
        let parent = self.find_best_parent_cell(evt);
        if parent.is_some() {
            ...
            let max_idx = std::cmp::min(SLAB_DEPTH-1, in_cell.len());
...
mod tests {
    ...
    fn test_spiral_insert() {
        ...
        let check = store.add_and_match_feature(&first_node);
        ...
        for i in 0..MAX_IDX {
            ...
            let check = store.add_and_match_feature(&node);
...
//                }
    ...
    fn test_distant_feature_insertion() {
        ...
        node1.row = 320;
        node1.col = 320;
        let check1 = store.add_and_match_feature(&node1);
        ...
        let check2 = store.add_and_match_feature(&node2);
